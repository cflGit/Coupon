...
- (void)viewDidLoad
{
    ...
    _viewNaviBar = [[CustomNaviBarView alloc] initWithFrame:Rect(0.0f, 0.0f, [CustomNaviBarView barSize].width, 64)];
...
//    int padding = 0;
//    //为 导航视图 代码添加约束
//    [_viewNaviBar mas_makeConstraints:^(MASConstraintMaker *make) {
//        make.top.equalTo(superview.mas_top).offset(padding); //with with
//        make.left.equalTo(superview.mas_left).offset(padding); //without with
...
-(void)setButtonStyle:(UIButtonStyle)style andTitle:(NSString *)titleStr titleColor:(UIColor *)titleColor {
    if (style == UIButtonStyleLeft) {
        UIButton *leftBtn  =   [CustomNaviBarView createNormalNaviBarBtnByTitle:titleStr target:self action:@selector(leftBarButtonClick:)];
        [self setNaviBarLeftBtn:leftBtn];
    }else {
        UIButton *rightBtn  =   [CustomNaviBarView createNormalNaviBarBtnByTitle:titleStr target:self action:@selector(rightBarButtonClick:)];
        [self setNaviBarRightBtn:rightBtn];
