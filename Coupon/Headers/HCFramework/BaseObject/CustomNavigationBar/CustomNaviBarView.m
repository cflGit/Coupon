...
{
    return Rect([UIScreen mainScreen].bounds.size.width-62, SCREENSize*22.0f, 60, SCREENSize*40.0f);
}

+ (CGSize)barBtnSize
{
    return Size(60.0f, SCREENSize*40.0f);
}

+ (CGSize)barSize
{
    return Size([UIScreen mainScreen].bounds.size.width,SCREENSize* 64.0f);
}

+ (CGRect)titleViewFrame
{
    
    return Rect(([UIScreen mainScreen].bounds.size.width-200)/2, SCREENSize*22.0f, 200.0f, SCREENSize*40.0f);
}
...
//    _btnBack = [[self class] createImgNaviBarBtnByImgNormal:@"search_back.png" imgHighlight:@"search_back.png" target:self action:@selector(btnBack:)];
    ...
    UIImage *NaviBar_BgImage = [UIImage createImageWithColor:[UIColor whiteColor]];
...
- (void)setLeftBtn:(UIButton *)btn
{
    ...
    if (_btnLeft)
    {
        _btnLeft.frame = Rect(0, SCREENSize*22.0f, [[self class] barBtnSize].width, [[self class] barBtnSize].height);
