...
#define RECT_NaviBar                            Rect(0, SCREENSize*22.0f, [UIScreen mainScreen].bounds.size.width, 40)
#define RECT_SearchBarFrame                     Rect(0.0f, 0.0f, [CustomNaviBarView rightBtnFrame].origin.x-2.0f, NaviBarHeight)
#define RECT_SearchBarCoverCancelBtnFrame       Rect(0.0f, 0.0f, [UIScreen mainScreen].bounds.size.width, NaviBarHeight)
...
#define RECT_secachBarFrame Rect(60, (NaviBarHeight-35)/2, (ScreenWidth-120), 35)
...
- (void)initUI
{
    ...
    _searchBar = [[UISearchBar alloc] initWithFrame:RECT_secachBarFrame];
    ...
    //去除背景颜色
    [self searchBarClear:_searchBar];
    _searchBar.layer.masksToBounds = YES;
    _searchBar.layer.borderWidth =1;
    _searchBar.layer.borderColor =[UIColor darkGrayColor].CGColor;
    ...
    _viewBlackCover.backgroundColor = [UIColor darkGrayColor];
    _viewBlackCover.alpha = 0.8;
...
- (void)startWorking
{
    ...
    if (_bIsCoverTitleView)
    {
        ...
        _searchBar.frame = RECT_secachBarFrame;
    }
    else
    {
        _searchBar.frame = RECT_secachBarFrame;
...
- (void)showRecentTable:(BOOL)bIsShow
{
    if (bIsShow && _arrRecent && (_arrRecent.count > 0))
    {
        if (!_tableView)
        {
            _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, SCREENSize*64, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height-SCREENSize*(64+49)) style:UITableViewStylePlain];
            [_viewCtrlParent.view addSubview:_tableView];
            [UtilityFunc resetScrlView:_tableView contentInsetWithNaviBar:YES tabBar:NO];
            
            UIView *footer = [[UIView alloc] initWithFrame:Rect(0.0f, 0.0f, [UIScreen mainScreen].bounds.size.width, 240.0f)];
            footer.backgroundColor = [UIColor clearColor];
            _tableView.tableFooterView = footer;
            UIView *header = [[UIView alloc] initWithFrame:Rect(0.0f, 0.0f, [UIScreen mainScreen].bounds.size.width, 20)];
