...
char *_encode(char *src, char **dst, unsigned char *key)
{
	...
	if((xxtea = xxtea_encrypt((unsigned char *) src, key, &xxtea_length)) == NULL)
		return NULL;
	if((result = msbase64_encode(xxtea, xxtea_length, &base64_length)) == NULL) {
		free(xxtea);
		return NULL;
	}
...
char *_decode(char *src, char **dst, unsigned char *key)
{
	...
	if((xxtea = mbase64_decode((unsigned char *)src, base64_length, &xxtea_length)) == NULL)
		return NULL;
